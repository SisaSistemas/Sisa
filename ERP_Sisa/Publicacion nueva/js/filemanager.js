...
if($.urlParam('langCode') != 0 && file_exists ('../js/languages/'  + $.urlParam('langCode') + '.js')) culture = $.urlParam('langCode');
var lg = [];
$.ajax({
  url: '../js/languages/'  + culture + '.js',
  async: false,
  dataType: 'json',
...
var bindToolbar = function(data){
	...
	//	$('#fileinfo').find('button#select').hide();s
	//} else {
			...
			$.ajax({
				...
				success: function(result){
					if(result['Code'] == 0){
						...
						$('#fileinfo').find('button#download').unbind();
						bindToolbar(data);
