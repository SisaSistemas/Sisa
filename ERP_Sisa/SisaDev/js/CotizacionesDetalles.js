    ...
    $.ajax({
        ...
        success: function (data, textStatus) {
            ...
            $.each(json, function (index, value) {
                ...
                IdUsuarioVendido = value.VendidoPor
               // $("#cmbContactos option[value='" + idContacto + "']").prop('selected', true);                
                ...
                $("#cmbUsuariosVendido").val(value.IdUsuarioVendido);
                $('#cmbUsuariosVendido').selectpicker('refresh');

                $("#cmbSucursal").val(value.Sucursal);
...
function CargarSucursales() {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        type: 'POST',
        url: 'CotizacionesDetalles.aspx/ObtenerSucursales',
        data: params,
        success: function (data, textStatus) {
            ...
            //debugger;
            $.each(json, function (index, value) {
                $("#cmbSucursal").append('<option value="' + value.idSucursa.toUpperCase() + '">' + value.CiudadSucursal.toUpperCase() + '</option>');
            });
            $('#cmbSucursal').selectpicker('refresh');
            ...
            //if (IdSolicitud != "null") {
            //    $("#cmbSucursal").val(idSucursa.toUpperCase());
            //    $('#cmbSucursal').selectpicker('refresh');
            //    //
...
function CargarUsuarios() {
    ...
    $.ajax({
        ...
        success: function (data, textStatus) {
            ...
            $('#cmbUsuariosVendido').html('');
            $('#cmbUsuariosCotizacion').html('<option value="-1">-- SELECCIONE USUARIO CREO --</option>');
            $('#cmbUsuariosVendido').html('<option value="-1">-- SELECCIONE USUARIO VENDIDO --</option>');
            $.each(json, function (index, value) {
                $("#cmbUsuariosCotizacion").append('<option value="' + value.Id + '">' + value.Nombre.toUpperCase() + '</option>');
                $("#cmbUsuariosVendido").append('<option value="' + value.Id + '">' + value.Nombre.toUpperCase() + '</option>');
            });
            $('#cmbUsuariosCotizacion').selectpicker('refresh');
            $('#cmbUsuariosVendido').selectpicker('refresh');
            $('#cmbUsuariosCotizacion').selectpicker('render');
            $('#cmbUsuariosVendido').selectpicker('render');
            if (IdCotizaciones != '0') {
                //$("#cmbUsuariosCotizacion option[value='" + IdUsuarioCreo + "']").prop('selected', true);
                $("#cmbUsuariosCotizacion").val(IdUsuarioCreo);
                $('#cmbUsuariosCotizacion').selectpicker('refresh');
                $("#cmbUsuariosVendido").val(IdUsuarioVendido);
                $('#cmbUsuariosVendido').selectpicker('refresh');
            }
        }
    });
...
$('#btnGuardarCotizacion').click(function () {
    ...
    swal({
        ...
        if ($('#cmbUsuariosCotizacion').val() == '-1') {
            swal("Selecciona Usuario Creado Por...");
            return;
        }
        if ($('#cmbUsuariosVendido').val() == '-1') {
            swal("Selecciona Usuario Vendido Por...");
            return;
        }
        ...
        var params = "{'idContacto': '" + $('#cmbContactos').val() +
            ...
            "','VendidoPor': '" + $('#cmbUsuariosVendido').val() +
                ...
                success: function (data, textStatus) {
                    ...
                    if (cadena[0] == "Cotizacion Modificada") {
                        $("#tblItemsCotizacion tbody tr").each(function (index) {
                            ...
                            var param = "{'item': '" + item +
                                "','Descripcion': '" + descripcion.replace(/"/g, '\\"').replace(",", "").replace(/[\n\r]/g, '') +
                                "','Cantidad': '" + cantidad +
                                "','Unidad': '" + unidad +
                                "','Precio': '" + precio +
                                "','Importe': '" + importe + "'}";
