$(document).ready(function () {
    ...
    function CargarCriterios() {
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            type: 'POST',
            url: 'OrdenCompraDetalle.aspx/CargaComboCriterios',
            data: params,
            success: function (data, textStatus) {
                ...
                $('#cmbCriterioOC').html('');
                $('#cmbCriterioOC').html('<option value="-1">-- SELECCION EL CRITERIO --</option>');
                $.each(json, function (index, value) {
                    $("#cmbCriterioOC").append('<option value="' + value.Id + '">' + value.Descripcion.toUpperCase() + '</option>');
                });
                $('#cmbCriterioOC').selectpicker('refresh');
                $('#cmbCriterioOC').selectpicker('render');
            }
        });
    }
        ...
        $.ajax({
            ...
            success: function (data, textStatus) {
                ...
                $.each(json, function (index, value) {
                    ...
                    if (value.IdCriterio) {
                        $('#cmbCriterioOC').val(value.IdCriterio);
                        $('#cmbCriterioOC').selectpicker('refresh');
                    }                   
        ...
        $.ajax({
            ...
            success: function (data, textStatus) {
                ...
                $.each(json, function (index, value) {
                    ...
                    if (_pintarRenglon != "verde") {
                        nodoTRAgregar += "  <td><span class='btn btn-success icon_check recibir'></span>";

                        if (rolusuario != "Recepcion almacen") {                            
                            nodoTRAgregar += "&nbsp;&nbsp;<span class='btn btn-danger icon_close eliminaritemoc'></span>";
                        }
                        nodoTRAgregar += "&nbsp;&nbsp;<span class='btn btn-success icon_pencil modificaritemoc'></span></td>";
                    }
                    else {
                        nodoTRAgregar += "  <td></td>";
                    }
    ...
    $("#txtComentarios").keydown(function (e) {
        if (e.key == "'" || e.key == '"') {
    ...
    $("#cmbProyectos").change(function () {
        ...
        if (idProyecto != null) {
            ...
            $.ajax({
                async: true,
                url: "OrdenCompraDetalle.aspx/BuscarProyectoInsumo",
                dataType: "json",
                data: params,
                type: "POST",
                contentType: "application/json; charset=utf-8",
                success: function (data, textStatus) {
                    ...
                    //var json = $.parseJSON(data.d);
                    $('#cmbTipoOC').html('');
                    $('#cmbTipoOC').html('<option value="-1">-- SELECCIONE TIPO OC --</option>');
                    $('#cmbTipoOC').append('<option value="Material">Material</option>');
                    $('#cmbTipoOC').append('<option value="Equipo">Equipo</option>');
                    $('#cmbTipoOC').append('<option value="ManoObra">Mano de Obra</option>');
                    if (data.d == 'INSUMO') {
                        $('#cmbTipoOC').append('<option value="GastoGeneral">Gasto General</option>');
                    }

                    //$.each(json, function (index, value) {
                    //    $("#cmbTipoOC").append('<option value="' + value.Id.toUpperCase() + '">' + value.Nombre.toUpperCase() + '</option>');
                    //});
    ...
    $('#btnAgregar').click(function () {
        ...
        nodoTRAgregar += "  <td>" + $('#txtComentarios').val().replace(/\\"/g, '').replace(/"/g, '').replace(/'/g, '').replace(/[\n\r]/g, '') + "</td>";
    ...
    $('#btnGuardar').click(function () {
        ...
        swal({
            ...
            if ($('#cmbCriterioOC').val() == '-1') {
                swal("Selecciona tipo de criterio...");
                return;
            }
                    ...
                    if (idOrdenCompra != 0) {
                        ...
                        $("#tblItems tr:has(td)").each(function (i, tr) {
                            ...
                            $(tr).find('td').each(function (j, td) {
                                row[header[j]] = $(td).html().replace(/"/g, '\\"').replace(",", "").replace(/[\n\r]/g, '') ;
                            });
                            rows.push(row);
                        });
    ...
    $('#btnRecibirGlobalOC').click(function () {
        ...
        swal({
            ...
            if (IdOrdenCompra != 0) {
                ...
                $.ajax({
                    async: true,
                    url: 'OrdenCompraDetalle.aspx/RecibeTodoMaterial',
                    dataType: "json",
                    data: params,
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
    ...
    $(document).on('click', '.modificaritemoc', function (event) {
        if (IdOrdenCompra != '0') {
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                type: 'POST',
                url: 'OrdenCompraDetalle.aspx/ObtieneInfoItem',
                data: params,
                success: function (data, textStatus) {
                    var json = $.parseJSON(data.d);
                    var nodoTRAgregar = "";
                    $.each(json, function (index, value) {
                        nodoTRAgregar += "<tr>";
                        //nodoTRAgregar += "  <td>" + _item + "</td>";
                        nodoTRAgregar += "  <td><input type='text' class='form-control' id='txtItemOC'  reaonly value='" + value.Item + "' /></td>";
                        nodoTRAgregar += "  <td><input type='text' class='form-control' id='txtComentariosItemOC'  required value='" + value.Comentarios + "' /></td>";
                        nodoTRAgregar += "  <td><input type='text' class='form-control' id='txtCantidadItemOC'  required value='" + formato_numero(value.Cantidad, 2, '.', ',') + "' /></td>";
                        nodoTRAgregar += "  <td><input type='text' class='form-control' id='txtPrecioItemOC'  required value='" + formato_numero(value.Precio, 2, '.', ',') + "' /></td>";
                        nodoTRAgregar += "  <td><input type='text' class='form-control' id='txtTiempoEntregaItemOC'  required value='" + value.TiempoEntrega + "' /></td>";
                        nodoTRAgregar += "</tr>";
                        
                    });
                    $('#TablaItemOC tbody').append(nodoTRAgregar);
                 }
            });

            

            $("#EditarItemModal").modal();
    ...
    $("#btnActualizaItemOC").click(function () {
        if ($('#txtCantidadItemOC').val() == "" || $('#txtPrecioItemOC').val() == "") {
            swal("Favor de ingresar los datos de materiales correctamente...");
            return;
        }

        var _precio1 = $('#txtPrecioItemOC').val();
        _precio1 = _precio1.split(',').join('');

        var _item = $('#txtItemOC').val();
        var _comentarios = $('#txtComentariosItemOC').val();
        var _cantidad = formato_numero($('#txtCantidadItemOC').val().replace(',', ''), 2, '.', ',');
        var _precio = formato_numero(_precio1, 2, '.', ',');
        var _tiempoentrega = $('#txtTiempoEntregaItemOC').val();
        var _iva = $('#txtIVA').val();

        var params = "{'IdOrdenCompra': '" + IdOrdenCompra + "','Item': '" + _item + "', 'Comentarios': '" + _comentarios + "', 'Cantidad': '" + _cantidad + "', 'Precio': '" + _precio + "', 'TiempoEntrega': '" + _tiempoentrega + "', 'Iva': '" + _iva + "'}";
        //debugger;
        $.ajax({
            dataType: 'json',
            async: true,
            contentType: "application/json; charset=utf-8",
            type: 'POST',
            url: 'OrdenCompraDetalle.aspx/ModificaItem',
            data: params,
            success: function (data, textStatus) {
                swal({
                    title: 'Guardado Correctamente!',
